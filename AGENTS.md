...
- `src/pages/` contains Astro pages and API-style routes; `src/pages/index.astro` is intentionally thin and calls `getChannelInfo()`. Pagination routes are `/before/[cursor]` and `/after/[cursor]`. Content pages: `/about` (requires `LINKS` env; `/links` redirects here), `/tags` (lists the tags configured in `TAGS`, or every discovered tag when that env is empty, and is linked from the nav whenever either source is non-empty), `/archive` (always available). Search: `/search/result` (SSR page) and `/search/[q]` (API). Feeds/sitemap: `/rss.xml`, `/rss.json`, `/sitemap.xml`, `/sitemap/[cursor].xml`. Other API routes: `/static/[...url]` (proxy), `/rules/prefetch.json`, `/site.webmanifest`.
...
- `ChannelInfo.posts` is **newest-first**: `posts[0]` is the newest post on the page and `posts[posts.length - 1]` the oldest (`getChannelInfo` reverses Telegram's markup order and does not sort). That is why `PostsPage` uses `posts[0].id` as the newer cursor and the last id as the older one. Anything that needs a neighbouring post must compare ids numerically rather than index into the array — message ids have gaps where posts were deleted, so `id ± 1` is not a neighbour. `src/pages/posts/[id].astro` does exactly this to build the previous/next links from the pages either side of the current id.
...
- Telegram wraps a media-group caption in an **extra element carrying the same class** (`<div class="tgme_widget_message_text"><div class="tgme_widget_message_text">…`). `extractPost` therefore keeps only the outermost `.tgme_widget_message_text` matches: cheerio's `.text()` walks the whole selection and would otherwise read such a caption twice, doubling `post.title`/`post.text` and every image `alt` derived from them. Note the asymmetry that hid this — `.text()` concatenates all matched elements while `.html()` renders only the first, so the rendered post body looked correct while the title did not.
